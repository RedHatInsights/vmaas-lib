...
func TestPaginate(t *testing.T) {
	...
	// empty slice
	subslice, paginationDetails := Paginate([]int{}, PaginationRequest{1, 2})
	...
	subslice, paginationDetails = Paginate(slice, PaginationRequest{0, -1})
	assert.Equal(t, len(slice), len(subslice))
	assert.Equal(t, 1, paginationDetails.PageNumber)
	assert.LessOrEqual(t, paginationDetails.PageSize, 5000)

	// usual case
	subslice, paginationDetails = Paginate(slice, PaginationRequest{2, 2})
	...
	// the last page
	subslice, paginationDetails = Paginate(slice, PaginationRequest{2, 3})
