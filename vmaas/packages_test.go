...
func TestFilterInputPkgs(t *testing.T) {
	c := mockCache()
	pkgs := []string{"badnevra", "badnevra", "bash-4.2.46-20.el7_2.x86_64.rpm", "python-perf-3.10.0-693.el7.x86_64", "vim-common-2:7.4.160-1.el7.x86_64.rpm"}
	req := &PackagesRequest{
		Packages:   pkgs,
		ThirdParty: false,
	}
...
func TestLoadPackageDetails(t *testing.T) {
	...
	pd, ok := packageDetails["bash-4.2.46-20.el7_2.x86_64.rpm"].(PackageDetail)
	assert.True(t, ok)
	...
	packageDetails = c.loadPackageDetails([]string{}, packages)
	assert.Equal(t, 1, len(packageDetails))
	pd, ok = packageDetails["bash-4.2.46-20.el7_2.src.rpm"].(PackageDetail)
	assert.False(t, ok)
}
