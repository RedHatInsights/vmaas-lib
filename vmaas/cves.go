...
func filterInputCves(c *Cache, cves []string, req *CvesRequest) []string {
	...
	for _, cve := range cves {
		...
		if req.ModifiedSince != nil && cveDetail.ModifiedDate != nil && cveDetail.ModifiedDate.Before(*req.ModifiedSince) {
			continue
		}
		if req.PublishedSince != nil && cveDetail.PublishedDate != nil && cveDetail.PublishedDate.Before(*req.PublishedSince) {
			continue
		}
